...
int main(){
    int n;
    printf("Give the value of size of matrix.");
    scanf("%d", &n);

    int mat[n][n];
    printf("Write down the matrix below.\n");
    for(int i =0 ;i< n ;i++){
        for (int j =0;j<n;j++){
            scanf("%d", &mat[i][j]);
        }
...
//printf("%d\n", max);
printf("Row with max average is -> ");
printf("%d\n", j+1);
...
//printf("%d\n", min);
printf("Column with minimum average is -> ");
printf("%d\n", k+1);
...
mat[a][b]=mat[c][d];
mat[c][d]=temp;
printf("Matrix after swapping minimum and maximum \n");
for(int i =0; i< n ;i++){
    for(int j =0;j<n ;j++){
        printf("%d ", mat[i][j]);
    }
...
// Q4
printf("Give the value if x and y such that xth and yth rows and columns would be interchanged\n");
int x=0, y=0;
scanf("%d", &x);
scanf("%d", &y);
printf("\n");
